...
%global commit 0f6704f961642daeeda99e95a034b40857bda627
...
This package provides the source libraries for %{name}.

%package fish-completion
Summary:    Docker fish completion files
...
logrotate is assumed to be installed on containers for
this to work, failures are silently ignored.

%package vim
Summary:    Docker vim syntax highlighting files
Requires:   %{name} = %{version}-%{release}
Requires:   vim
...
Summary:    Docker zsh completion files
Requires:   %{name} = %{version}-%{release}
Requires:   zsh
...
install -p -m 755 bundles/%{version}-dev/dynbinary/%{name}-%{version}-dev %{buildroot}%{_bindir}/%{name}

# install dockerinit
install -d %{buildroot}%{_libexecdir}/%{name}
install -p -m 755 bundles/%{version}-dev/dynbinary/%{name}init-%{version}-dev %{buildroot}%{_libexecdir}/%{name}/%{name}init
...
# delete pem certs in source
rm integration/fixtures/https/*.pem
...
%changelog
* Wed Mar 04 2015 Lokesh Mandvekar <lsm5@fedoraproject.org> - 1.5.0-18.git0f6704f
- move from docker-io to docker
